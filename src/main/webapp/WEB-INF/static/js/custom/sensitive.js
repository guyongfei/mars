var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#inner_table').bootstrapTable({
            url: contextPath + '/management/sensitiveword',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
    ...
    oTableInit.queryParams = function (params) {
        var temp = {
            ...
            wordLike: $("#txt_search").val().trim() //模糊查询
        };
        return temp;
    };
    return oTableInit;
...
window.operateEvents = {
    'click .aaa': function (e, value, row, index) {
        bootbox.confirm("确认提交", function (result) {
            if (result) {
                $.ajax({
                    url: contextPath + "/management/sensitiveword/" + row.id,
                    type: "delete",
...
/*
 ...
 $.ajax({
 url: contextPath + "/management/sensitiveword",
 type: "post",
 contentType: "application/json;charset=UTF-8",
