...
CREATE TABLE `record_user_tx` (
  ...
  `user_tx_status` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '状态(0-初始状态,11-验证成功,11-验证失败（to不是平台地址）,12-验证失败（from不匹配）,13-验证失败（金额不匹配）,2-交易失败,3-交易不存在',
  `platform_tx` varchar(68) NOT NULL DEFAULT '' COMMENT '打币交易号',
  `eth_fee` decimal(20,10) NOT NULL DEFAULT '0.0000000000' COMMENT '手续费',
  `platform_tx_status` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '打币交易状态(0-初始状态,1-成功,2-失败,3-交易作废)',
